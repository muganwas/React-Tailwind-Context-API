...
const MessageSummary = ({ message, active, _onClick, inMessage }) => {
    return (
        <div onClick={_onClick && _onClick} className={`flex flex-col w-full cursor-pointer summary-container ${active ? 'active' : ''} ${!inMessage ? 'py-5 px-5' : 'py-2 px-2'}`}>
            <div className='flex flex-row my-1 justify-start items-center'>
                ...
                <span className={`flex flex-grow text-md font-medium ${!inMessage && 'ml-2'}`}>{message.issue} issue</span>
            ...
            <div className='flex text-sm my-1 justify-between items-center'>
                <div className='flex text-sm'>
                    ...
                    <span className='text-primary ml-1'>{message.from}</span>
                </div>
